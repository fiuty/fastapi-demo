...
`AgentscopeService.interrupt()` 根据智能体当前状态自动选择中断策略：

| 场景 | 智能体状态 | 中断方式 | SDK 清理路径 |
|---|---|---|---|
| **运行中** | 正在执行 `reply_stream` (推理/工具执行) | `task.cancel()` 取消承载流的 asyncio Task | Agent 内部捕获 `CancelledError`，产出 INTERRUPTED 结束事件 |
| **暂停中** | 等待用户确认 / 外部工具执行 (HITL) | 向 `reply_stream` 传入 `UserInterruptEvent` | Agent 清理待处理工具调用，产出 INTERRUPTED 结束事件 |
